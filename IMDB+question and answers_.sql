...
SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS DATE_PBUBLISHED_NULL,
SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS DURATIO_NULL,
SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS COUNTRY_NULL,
SUM(CASE WHEN  worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS WORLWIDE_INCOME_NULL,
...
-- query format for the first part:
...
-- query format for the Second part:
...
SELECT COUNT(DISTINCT id) AS num_of_movies_produced, 
year
FROM movie
...
SELECT genre,COUNT(DISTINCT movie_id) AS num_of_movies
FROM genre
INNER JOIN movie 
ON movie_id = id
GROUP BY genre
...
INNER JOIN ratings
ON id = movie_id
WHERE date_published BETWEEN '2018-04-1' AND'2019-04-1' AND median_rating = 8 
GROUP BY median_rating;
